...
export default function BrandProductsPage() {
  ...
  const getCategoryLabel = (category: string) => {
    switch (category) {
      case 'Tops': return v('Tops', 'Áo');
      case 'Bottoms': return v('Bottoms', 'Quần');
      case 'Dresses': return v('Dresses', 'Váy đầm');
      case 'Outerwear': return v('Outerwear', 'Áo khoác');
      case 'Accessories': return v('Accessories', 'Phụ kiện');
              ...
              {filteredProducts.map((product) => (
                <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-500">
                    {lang === 'vi' ? product.categoryVi : getCategoryLabel(product.category)}
                  </td>
