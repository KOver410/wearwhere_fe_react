...
import { styleSuggestions, type StyleSuggestion } from '@/shared/data/wardrobeMockData';
                ...
                <div key={suggestion.id} className="bg-white overflow-hidden" style={{ borderRadius: '10px', border: '1px solid #e0d8cf', boxShadow: '0px 1px 3px rgba(0,0,0,0.1), 0px 1px 2px -1px rgba(0,0,0,0.1)' }}>
                  <div className="flex flex-col md:flex-row">
                    <div className="md:w-72 flex-shrink-0">
                      <ImageWithFallback src={suggestion.image} alt={suggestion.title} className="w-full h-56 md:h-full object-cover" />
                    </div>
                    <div className="flex-1 p-6">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div>
                          <h2 style={{ fontSize: '20px', fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: '#0d0d0d', textTransform: 'uppercase' as const }}>{suggestion.title}</h2>
                          <div className="flex items-center gap-3 mt-1">
                            <span className="px-2.5 py-0.5 capitalize" style={{ fontSize: '12px', color: '#4a4a4a', borderRadius: '9999px', backgroundColor: '#f3f0eb' }}>{suggestion.style}</span>
                            <span className="px-2.5 py-0.5" style={{ fontSize: '12px', color: '#4a4a4a', borderRadius: '9999px', backgroundColor: '#f3f0eb' }}>{suggestion.occasion}</span>
                            {WeatherIcon && suggestion.weather && (
                              <span className="flex items-center gap-1 px-2.5 py-0.5" style={{ fontSize: '12px', color: '#8B6914', borderRadius: '9999px', backgroundColor: '#FAF0DC' }}>
                                <WeatherIcon className="w-3 h-3" />{suggestion.weather}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <p style={{ fontSize: '14px', color: '#4a4a4a', lineHeight: '1.6', marginBottom: '16px' }}>{suggestion.description}</p>
                        ...
                        {suggestion.items.map(item => (
                          <Link key={item.id} to={`/product/${item.id}`} className="flex items-center gap-2.5 px-3 py-2 hover:bg-[#e0d8cf] transition-colors flex-shrink-0" style={{ borderRadius: '10px', backgroundColor: '#f3f0eb' }}>
                            <div className="relative">
                              <ImageWithFallback src={item.image} alt={item.name} className="w-10 h-10 object-cover" style={{ borderRadius: '4px' } as any} />
                              {item.owned && (
                            ...
                            <div>
                              <p className="line-clamp-1" style={{ fontSize: '12px', fontWeight: 600, color: '#0d0d0d' }}>{item.name}</p>
                              <p style={{ fontSize: '11px', color: item.owned ? '#e2b93b' : '#d41c1c' }}>
                                {item.owned ? v('In wardrobe', 'Trong tủ đồ') : `$${item.price}`}
                              </p>
                            </div>
                          </Link>
                        ))}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {!allOwned && (
                            <span style={{ fontSize: '12px', color: '#d41c1c' }}>
                              {v(`Need $${suggestion.items.filter(i => !i.owned).reduce((s, i) => s + i.price, 0)} more`, `Cần thêm $${suggestion.items.filter(i => !i.owned).reduce((s, i) => s + i.price, 0)}`)}
                            </span>
                          )}
                        </div>
